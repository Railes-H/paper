...
- 新增论文上传识别：支持上传 `docx`、`pdf`、`txt`、`md`，自动识别标题、摘要、关键词和估算字数。
...
- 文件管理：管理论文母版、投稿版、回执、录用通知、拒稿邮件、审稿意见等文件链接或本地路径。
...
- 线上上传识别可以完成文本解析，但当前不会永久保存原文件。
- 线上文件链接会显示为 `vercel-memory://...`。
- 如果需要长期保存上传的 Word/PDF 文件，建议后续接入 Vercel Blob、S3、Cloudflare R2 或其他对象存储。
